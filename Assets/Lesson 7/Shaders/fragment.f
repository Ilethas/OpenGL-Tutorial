...
void main()
{
	...
	for (int i = 0; i < MAX_LIGHTS; i++)
	{
		if (lights[i].active)
		{
			vec4 bounceColor = vec4(calcLight(i, shadingType), 0.0);
			float brightness = 0.2126 * bounceColor.r + 0.7152 * bounceColor.g + 0.0722 * bounceColor.b;
			bounceColor =  floor(brightness * levels) / levels * bounceColor;
